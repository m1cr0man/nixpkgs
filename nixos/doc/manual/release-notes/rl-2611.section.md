...
- `systemd.nspawn.<name>.settings` has been introduced to replace the individual `execConfig`, `filesConfig` and `networkConfig` options. A rename notice has been added so that existing configurations continue to work.
